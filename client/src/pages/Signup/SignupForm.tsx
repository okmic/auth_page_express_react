...
export const SignUpForm: React.FC<SignUpFormProps> = ({
  ...
  const validateField = (name: string, value: string): string | undefined => {
    switch (name) {
      ...
      case 'confirmPassword':
        ...
        if (value !== formData.psw) return 'Пароли не совпадают'
        break
    }
    return undefined
  }
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === 'psw' && touched.has('confirmPsw')) {
      const confirmError = validateField('confirmPsw', formData.confirmPsw)
      setErrors(prev => ({
        ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const fieldsToValidate = ['vin', 'name', 'email', 'psw', 'confirmPsw']
      ...
      <div className="space-y-2">
        <label 
          ...
          className={`
            ...
            ${errors.confirmPsw ? 'text-[#FF3B30]' : ''}
            ${!focusedField && !errors.confirmPsw ? 'opacity-80' : ''}
          `}
        >
        ...
        <div className="relative">
          <input
            ...
            className={`
              ...
              ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}
              ${!errors.confirmPsw && !focusedField ? 'hover:border-[#00E5B0]/30 hover:bg-white/15' : ''}
            `}
          />
          <div className="absolute right-6 top-1/2 transform -translate-y-1/2">
            <div className={`
              w-2 h-2 rounded-full
              transition-all duration-300
              ${formData.confirmPsw && !errors.confirmPsw && formData.psw === formData.confirmPsw
                ? 'bg-[#00E5B0] animate-pulse' 
                : errors.confirmPsw
                  ? 'bg-[#FF3B30]' 
                  : 'bg-[#F5F0E8]/30'
